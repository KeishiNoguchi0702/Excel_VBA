...
Sub mysub4_32()
    ...
    With rng
        Set rng = Sheet1.Range("B1")
        .Value = 1000
        .Font.Bold = True
        .Interior.Color = RGB(255, 255, 0)
    End With
    
   '本プロシージャを実行して反映するのは､セルB1ではなくセルA1｡
   'Withステートメントは､Withブロック内で使用する一時的な変数が用意される｡代入は最初の一度のみ行われ､Withブロック内で行われることはない｡
   'このため､WithステートメントとForステートメントを組み合わせる場合は､WthブロックにForブロックを入れたほうが､コンピュータの処理速度は向上する
